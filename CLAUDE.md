...
MediaFlow is a YouTube-like video upload and adaptive streaming platform, now being built out as a hardcore distributed-systems project. A user uploads an MP4, it gets transcoded to multi-quality HLS with FFmpeg, and plays back via hls.js. The MVP pipeline (Milestones 0–3) is complete; Phase 2 (Milestones 4–9: transactional outbox, job leases/retries/DLQ, presigned multipart uploads, fan-out transcoding, signed manifests + edge cache, OpenTelemetry, load/chaos testing) is designed in `MEDIAFLOW_PLAN.md` — read the relevant milestone section there before implementing it. Status and milestone checklists are in `PROGRESS.md` (update it after completing tasks — see its "Update Rules" section). Repo conventions are in `AGENTS.md`.

Phase 2 engineering rules (from the plan): never dual-write DB-then-publish — go through the outbox; every queue consumer must be idempotent (delivery is at-least-once); every video state must have a path to `ready` or `failed` via retry, timeout, or reaper; each milestone is proven with a failure drill or measurement, not just passing tests.
