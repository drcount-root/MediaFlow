...
// Package integration exercises the worker against real Postgres, RabbitMQ, and
// MinIO started with testcontainers-go, plus a real ffmpeg/ffprobe on PATH. It
// is gated behind the `integration` build tag:
//
//	go test -tags integration ./...
//
// A Docker daemon and ffmpeg/ffprobe must be available.
package integration
...
func truncateAll(t *testing.T, db *sql.DB) {
	...
	_, err := db.ExecContext(context.Background(),
		`TRUNCATE videos, video_jobs, video_variants, video_events RESTART IDENTITY CASCADE`)
	if err != nil {
